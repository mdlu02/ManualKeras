...
class Model(ABC):
    ...
    def evaluate(self, x, y, batch_size):
        ...
        for b1 in range(batch_size, x.shape[0] + 1, batch_size):
            b0 = b1 - batch_size
            batch_metrics = self.batch_step(x[b0:b1], y[b0:b1], training=False)
            update_metric_dict(batches, batch_metrics)
        update_metric_dict(agg_metrics, batches)
        return agg_metrics

    @abstractmethod
    def call(self, inputs):
        """Implemented in SequentialModel class in main.py"""
        return
